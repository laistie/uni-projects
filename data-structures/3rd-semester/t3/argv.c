...
struct checkedPaths{
    bool givenPathIn, pathIn, givenPathOut, pathOut, fileGeo, fileQry, epsilon;
}; 
...
void setCheckedPaths(CheckedPaths checkedPaths){
    ...
    aux->givenPathIn = aux->pathIn = aux->givenPathOut = aux->pathOut = aux->fileGeo = aux->fileQry = aux->epsilon = false;
}
...
void setArgv(int argc, char** argv, void* paths, CheckedPaths checkedPaths){
    ...
    while(i < argc){ 
        if(strcmp(argv[i], "-e") == 0){ //PATH PARA DIRETÓRIO BASE DE ENTRADA (BED)
            ...
            setEpsilon(paths, number);
            setEpsilonTrue(checkedPaths);
        }
        i++;
    }
    ...
    if(!getCheckedEpsilon(checkedPaths)){
        setEpsilon(paths, 0);
        setEpsilonTrue(checkedPaths);
    }
}