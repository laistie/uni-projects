...
void setArgv(int argc, char** argv, void* paths, void* checkedPaths){
    ...
    while(i < argc){ 
        if(strcmp(argv[i], "-e") == 0){ //PATH PARA DIRETÓRIO BASE DE ENTRADA (BED)
            ...
            char* auxGeo = calloc(strlen(argv[i]) + 1, sizeof(char)); //GIVEN GEO
            strcpy(auxGeo, argv[i]);
            char* tempGeo = calloc(strlen(argv[i]) + 1, sizeof(char)); //GEO FILENAME
            if(checkRelGeo(paths) == true){
                char* geoArr = calloc(strlen(auxGeo) + 1, sizeof(char));
                int last = 0;
                int begin = 0;
                if(auxGeo[0] == '.' || auxGeo[0] == '/'){ //REMOVING RELATIVE PATH STUFF
                    begin++;
                    if(auxGeo[1] == '/'){
                        begin++;
                    }
                }
                for(int j = begin; j < strlen(auxGeo); j++){ //REMOVING EXTENSION IF IT EXISTS
                    if(auxGeo[j] == '.'){
                        geoArr[last] = '\0';
                        break;
                    }
